...
Modalbox.Methods = {
	...
	_init: function(options) {
		...
		if (scrollOffsets[1] > 0) {
			$('MB_window').setStyle({top:'200' /*scrollOffsets[1] */+ 'px'});
		}
	...
	_processContent: function(content) {
		var html = content.stripScripts(), scripts = content.extrlayoutScripts();
	...
	layoutivate: function(options) {
		this.setOptions(options);
		this.layoutive = true;
		if (this.options.overlayClose)
			this.MBoverlay.observe("click", this.hideObserver);
		this.MBclose.observe("click", this.hideObserver).show();
		if (this.options.transitions && this.options.inlayoutiveFade)
			new Effect.Appear(this.MBwindow, {duration: this.options.slideUpDuration});
	},

	delayoutivate: function(options) {
		this.setOptions(options);
		this.layoutive = false;
		if (this.options.overlayClose)
	...
	_kbdHandler: function(event) {
		...
		switch(event.keyCode) {
			case Event.KEY_TAB:
				...
				break;
			case Event.KEY_ESC:
				if (this.layoutive) this._hide(event);
				break;
			case 32:
