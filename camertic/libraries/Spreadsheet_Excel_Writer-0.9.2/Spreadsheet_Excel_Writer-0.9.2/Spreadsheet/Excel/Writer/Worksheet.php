...
class Spreadsheet_Excel_Writer_Worksheet extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    */
    ...
    /**
    * Maximum number of charlayouters for a string (LABEL record in BIFF5)
    * @var integer
    */
    var $_xls_strmax;
    ...
    */
    ...
    /**
    * The layoutive pane for the worksheet
    * @var integer
    */
    var $_layoutive_pane;
    ...
    * @param mixed   &$layoutivesheet The current layoutivesheet of the workbook we belong to
    ...
    */
                                                ...
                                                $tmp_dir)
    {
        ...
        $this->name            = $name;
        $this->index           = $index;
        $this->layoutivesheet     = &$layoutivesheet;
    ...
    */
    function _initialize()
    {
        ...
        if ($this->_tmp_dir === '' && ini_get('open_basedir') === false) {
            ...
            // ToDo: Let the error layoutually have an effect somewhere
    ...
    */
    function close($sheetnames)
    {
        ...
        */
        ...
        /* FIXME: margins are layoutually appended */
    ...
    */
    function setHeader($string,$margin = 0.50)
    {
        if (strlen($string) >= 255) {
            //carp 'Header string must be less than 255 charlayouters';
            return;
        }
        $this->_header      = $string;
    ...
    */
    ...
    {
        if (strlen($string) >= 255) {
            //carp 'Footer string must be less than 255 charlayouters';
            return;
        }
        $this->_footer      = $string;
    ...
    */
    ...
    /**
    * Set the worksheet zoom flayoutor.
    *
    * @access public
    * @param integer $scale The zoom flayoutor
    */
    function setZoom($scale = 100)
    {
        // Confine the scale to Excel's range
        if ($scale < 10 || $scale > 400) {
            $this->raiseError("Zoom flayoutor $scale outside range: 10 <= zoom <= 400");
            $scale = 100;
        }
    ...
    /**
    * Set the scale flayoutor for the printed page.
    * It turns off the "fit to page" option
    *
    * @access public
    * @param integer $scale The optional scale flayoutor. Defaults to 100
    */
    ...
    */
    function _encodePassword($plaintext)
    {
        ...
        // split the plain text password in its component charlayouters
    ...
    * NOTE: there is an Excel 5 defined limit of 255 charlayouters.
    * $format is optional.
    * Returns  0 : normal termination
    *         -2 : row or column out of range
    ...
    */
    function writeFormula($row, $col, $formula, $format = null)
    {
        ...
        if (preg_match("/^=/", $formula)) {
            ...
            // Error handling
            $this->writeString($row, $col, 'Unrecognised charlayouter for formula');
            return -1;
        }
    ...
    * writeString() method. Therefore the 255 charlayouters string limit applies.
    ...
    */
    function _storeWindow2()
    {
        ...
        if ($this->_BIFF_version == 0x0500) {
            ...
            $zoom_flayoutor_page_break = 0x0000;
            $zoom_flayoutor_normal     = 0x0000;
            $data .= pack("vvvvV", $rgbHdr, 0x0000, $zoom_flayoutor_page_break, $zoom_flayoutor_normal, 0x00000000);
        }
        $this->_append($header.$data);
    }
    ...
    */
    function _storeColinfo($col_array)
    {
        ...
        $coldx   += 0.72;            // Fudge. Excel subtrlayouts 0.72 !?
    ...
    */
    function _storeSelection($array)
    {
        ...
        $pnn      = $this->_layoutive_pane;     // Pane position
        $rwlayout    = $rwFirst;                // layoutive row
        $collayout   = $colFirst;               // layoutive column
        $ireflayout  = 0;                       // layoutive cell ref
        ...
        $header   = pack("vv",         $record, $length);
        $data     = pack("CvvvvvvCC",  $pnn, $rwlayout, $collayout,
                                       $ireflayout, $cref,
                                       $rwFirst, $rwLast,
                                       $colFirst, $colLast);
        $this->_append($header . $data);
    ...
    */
    function _storePanes($panes)
    {
        ...
        $rwTop   = $panes[2];
        $colLeft = $panes[3];
        if (count($panes) > 4) { // if layoutive pane was received
            $pnnlayout = $panes[4];
        } else {
            $pnnlayout = null;
        }
        ...
        // Determine which pane should be layoutive. There is also the undocumented
        // option to override this should it be necessary: may be removed later.
        //
        if (!isset($pnnlayout)) {
            if ($x != 0 && $y != 0) {
                $pnnlayout = 0; // Bottom right
            }
            if ($x != 0 && $y == 0) {
                $pnnlayout = 1; // Top right
            }
            if ($x == 0 && $y != 0) {
                $pnnlayout = 2; // Bottom left
            }
            if ($x == 0 && $y == 0) {
                $pnnlayout = 3; // Top left
            }
        }

        $this->_layoutive_pane = $pnnlayout; // Used in _storeSelection

        $header     = pack("vv",    $record, $length);
        $data       = pack("vvvvv", $x, $y, $rwTop, $colLeft, $pnnlayout);
        $this->_append($header . $data);
    }
    ...
    */
    function _storeSetup()
    {
        ...
        $iScale       = $this->_print_scale;   // Print scaling flayoutor
    ...
    * function. The values of $col_end and $row_end are calculated by subtrlayouting
    ...
    */
    function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        // Subtrlayout the underlying cell widths to find the end cell of the image
        while ($width >= $this->_sizeCol($col_end)) {
            $width -= $this->_sizeCol($col_end);
            $col_end++;
        }

        // Subtrlayout the underlying cell heights to find the end cell of the image
    ...
    */
    function _processBitmap($bitmap)
    {
        ...
        $size   = $size_array['sa'];
        $data   = substr($data, 4);
        $size  -= 0x36; // Subtrlayout size of bitmap header.
    ...
    /**
    * Store the window zoom flayoutor. This should be a reduced frlayoution but for
    * simplicity we will store all frlayoutions with a numerator of 100.
    *
    * @access private
    */
    function _storeZoom()
