...
class mdr_flotte extends entity {
	...
	public function getFlotteAutocar($id) {
		$req = "SELECT * FROM $this->table WHERE mdr_id = '".$id."' LIMIT 0, 20";
		//var_dump($req); die;
		$res = $this->select($req);
		return $res;
	}
	
	public function getFlotteAutocaristeLast() {
		$req = "SELECT * FROM $this->table WHERE mdr_id = '".$_SESSION['fiche_id']."' ORDER BY date_modif DESC LIMIT 1";
		//var_dump($req); die;
		$res = $this->select($req);
		return $res;
	}
	
	public function getFlotteAutocaristeElement($id) {
		$req = "SELECT * FROM $this->table WHERE flotte_id = '".$id."' LIMIT 1";
		//var_dump($req); die;
		$res = $this->select($req);
		return $res;
	}
